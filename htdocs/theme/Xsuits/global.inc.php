...
:root {
    ...
    --colortextbackhmenu: #<?php print 000 //$colortextbackhmenu;
        ?>;
...
<?php if (getDolGlobalString('THEME_DARKMODEENABLED')) {
    print "/* For dark mode */\n";
...
--colorbackhmenu1: #3d3e40;
    ...
    /* Must be same than colorbackbody */
...
table.liste th.wrapcolumntitle.liste_titre_sel:not(.maxwidthsearch),
...
.dataTables_length label select {
    ...
    <?php if ( !getDolGlobalString('THEME_ADD_BACKGROUND_ON_INPUT')) {
        ?>border<?php echo !getDolGlobalString('THEME_SHOW_BORDER_ON_INPUT') ? '-bottom': '';
...
div.tabBar input,
div.tabBar input.flat,
div.tabBar textarea,
div.tabBar textarea.flat,
div.tabBar form.flat select,
div.tabBar select,
div.tabBar select.flat,
...
.logopublicpayment #dolpaymentlogo {
    ...
    /* better rendering on public page header */
}
...
#div_container_exportoptions fieldset,
#div_container_sub_exportoptions fieldset {
...
.refidno .button:not(.smallpaddingimp) {
...
.buttonDelete:hover:not(.nohover) {
...
.nowrapfordate {
    /* no wrap on all devices for dates */
    white-space: nowrap;
    display: inline-block;
}
...
.wordbreak {
    word-break: break-word;
    /* cut fist between word, inside word if not possible */
}
...
.marginleft2 {
    margin-<?php print $left;
    ?>: 2px;
}

.marginright2 {
    margin-<?php print $right;
    ?>: 2px;
}
...
<?php // Add a nowrap on smartphone, so long list of field used for filter are overflowed with clip
...
maxscreenheightless200 {
    max-height: <?php echo isset($_SESSION['dol_screenheight']) ? max(500, (int) $_SESSION['dol_screenheight'] - 200): 700;
    ?>px;
    /* we guarantee height of 500 */
}

.maxscreenheightless300 {
    max-height: <?php echo isset($_SESSION['dol_screenheight']) ? max(400, (int) $_SESSION['dol_screenheight'] - 300): 700;
    ?>px;
    /* we guarantee height of 500 */
}
...
span.widthcentpercentminusx:not(.select2-selection):not(.select2-dropdown),
...
span.widthcentpercentminusxx:not(.select2-selection):not(.select2-dropdown),
...
@media only screen and (max-width: 570px) {
    ...
    #tooltip {
        position: absolute;
        width: <?php print dol_size(300, 'width');
        ?>px;
    }
...
.menulogocontainer {
    margin: <?php echo $disableimages ? '0': '6';
    ?>px;
    margin-left: 11px;
    margin-right: 9px;
    padding: 0;
    height: <?php echo $disableimages ? '18': '35';
    ?>px;
    /* width: 100px; */
    max-width: 100px;
    vertical-align: middle;
...
@media only screen and (max-width: <?php echo !getDolGlobalString('THEME_ELDY_WITDHOFFSET_FOR_REDUC3') ? round($nbtopmenuentries * 47, 0) + 130 : $conf->global->THEME_ELDY_WITDHOFFSET_FOR_REDUC3; ?>px)
...
@media only screen and (min-width: 768px) and (max-width: <?php echo !getDolGlobalString('THEME_ELDY_WITDHOFFSET_FOR_REDUC3') ? round($nbtopmenuentries * 47, 0) + 130 : $conf->global->THEME_ELDY_WITDHOFFSET_FOR_REDUC3; ?>px)
...
div.fiche {
    margin-<?php print $left;
    ?>: <?php print(GETPOST('optioncss', 'aZ09')=='print'? 6 : (empty($conf->dol_optimize_smallscreen) ? '40' : '6'));
    ?>px;
    margin-<?php print $right;
    ?>: <?php print(GETPOST('optioncss', 'aZ09')=='print'? 6 : (empty($conf->dol_optimize_smallscreen) ? '36' : '6'));
    ?>px;
...
@media only screen and (max-width: 1000px) {
    div.fiche {
        margin-<?php print $left;
        ?>: <?php print(GETPOST('optioncss', 'aZ09')=='print'? 6 : ($dol_hide_leftmenu ? '6' : '20'));
        ?>px;
        margin-<?php print $right;
        ?>: <?php print(GETPOST('optioncss', 'aZ09')=='print'? 8 : 6);
        ?>px;
    }
...
div#tmenu_tooltip {
    <?php if (GETPOST('optioncss', 'aZ09')=='print') {
        ...
        ?>padding-<?php echo $right;
        ?>: <?php echo ($maxwidthloginblock - 10);
        ?>px;
        <?php
    }
...
div.tmenudiv {
    <?php if (GETPOST('optioncss', 'aZ09')=='print') {
        ...
        white-space: nowrap;
        border-top: 0px;
        border-<?php print $left;
        ?>: 0px;
        border-<?php print $right;
        ?>: 0px;
...
<?php if (in_array(getDolGlobalInt('THEME_TOPMENU_DISABLE_IMAGE'), array(2, 3, 4))) {
    ...
    ?>.tmenulabel:not(.menuhider),
    .tmenulabel:not(.menuhider)::before {
        display: none;
        /* opacity: 0; To show text after transition */
    }

    a.tmenuimage:not(.menuhider),
    a.tmenuimage:not(.menuhider)::before,
    div.tmenuimage:not(.menuhider),
    div.tmenuimage:not(.menuhider)::before,
    span.tmenuimage:not(.menuhider),
    span.tmenuimage:not(.menuhider)::before {
        font-size: 1.3em;
...
?><?php if (getDolGlobalInt('THEME_TOPMENU_DISABLE_IMAGE')==2) {
    ...
    ?>.tmenudiv:hover .tmenulabel:not(.menuhider),
    .tmenudiv:hover .tmenulabel:not(.menuhider)::before {
        display: block;
        position: relative;
        overflow: hidden;
        text-overflow: ellipsis;
    ...
    .tmenudiv:hover .tmenuimage:not(.menuhider),
    .tmenudiv:hover .tmenuimage:not(.menuhider)::before {
        /* For transition transition-delay: 1000ms;
	transition-property: all; */
...
?><?php if (getDolGlobalInt('THEME_TOPMENU_DISABLE_IMAGE')==3) {
    ...
    ?>li.tmenu:hover .tmenulabel:not(.menuhider),
    li.tmenu:hover .tmenulabel:not(.menuhider)::before {
        display: initial !important;
    }

    li.tmenu:hover .tmenuimage:not(.menuhider),
    li.tmenu:hover .tmenuimage:not(.menuhider):before {
...
<?php if (empty($dol_hide_topmenu)) {
    ...
    ?><?php // Add here more div for other menu entries. moduletomainmenu=array('module name'=>'name of class for div')
...
.login_table {
    ...
    ?>-webkit-box-shadow: 0 2px 23px 2px rgba(0, 0, 0, 0.2),
    0 2px 6px rgba(60, 60, 60, 0.15);
    box-shadow: 0 2px 23px 2px rgba(0, 0, 0, 0.2),
    0 2px 6px rgba(60, 60, 60, 0.15);
...
div.login_block_user {
    ...
    /* line-height: <?php echo $disableimages ? '25' : '52';
                        ?>px; */
    height: <?php echo $disableimages ? '25': '52';
    ?>px;
}
...
.userimgatoplogin img.userphoto {
    ...
    width: <?php echo $disableimages ? '26': '30';
    ?>px;
    height: <?php echo $disableimages ? '26': '30';
    ?>px;
    border-radius: 50%;
...
input.vmenusearchselectcombo[type=text] {
    width: <?php echo $leftmenuwidth - 10;
    ?>px !important;
}
...
#undertopmenu {
    background-repeat: repeat-x;
    margin-top: <?php echo ($dol_hide_topmenu ? '6' : '0');
    ?>px;
}
...
input.buttonreset.smallpaddingimp {
    font-size: 0.8em;
}
...
table.noborder:not(#tablelines):not(#tablelinesservice) tr:last-of-type td,
...
tr.liste_titre_topborder td {
    border-top-width: <?php echo $borderwidth;
    ?>px;
    border-top-color: var(--colortopbordertitle1);
    border-top-style: solid;
}
...
#tablelinesservice tr.liste_titre td,
...
#tablelines tr td,
#tablelinesservice tr td {
    height: unset;
}

/* Prepare to remove class pair - impair */

.noborder:not(.editmode)>tbody>tr:nth-child(even):not(.liste_titre):not(.nooddeven),
.liste>tbody>tr:nth-child(even):not(.liste_titre):not(.nooddeven),
div:not(.fichecenter):not(.fichehalfleft):not(.fichehalfright)>.border>tbody>tr:nth-of-type(even):not(.liste_titre):not(.nooddeven),
...
.noborder:not(.editmode)>tbody>tr:nth-child(odd):not(.liste_titre):not(.nooddeven),
.liste>tbody>tr:nth-child(odd):not(.liste_titre):not(.nooddeven),
div:not(.fichecenter):not(.fichehalfleft):not(.fichehalfright)>.border>tbody>tr:nth-of-type(odd):not(.liste_titre):not(.nooddeven),
...
@media only screen and (max-width: 767px) {
    ...
    .thumbstat {
        flex: 1 1 110px;
        margin-bottom: 8px;
        min-width: <?php echo isset($_SESSION['dol_screenwidth']) ? min(160, round((int) $_SESSION['dol_screenwidth'] / 2 - 20)): 150;
        ?>px;
    ...
    .thumbstat150 {
        flex: 1 1 110px;
        margin-bottom: 8px;
        min-width: <?php echo isset($_SESSION['dol_screenwidth']) ? min(160, round((int) $_SESSION['dol_screenwidth'] / 2 - 20)): 160;
        ?>px;
        /* on screen < 320, we guaranty to have 2 columns */
        max-width: <?php echo isset($_SESSION['dol_screenwidth']) ? min(161, round((int) $_SESSION['dol_screenwidth'] / 2 - 20)): 161;
        ?>px;
        /* on screen < 320, we guaranty to have 2 columns */
...
td.widthpictotitle,
.table-fiche-title img.widthpictotitle {
    width: 38px;
    text-align: <?php echo $left;
    ?>;
}
...
div.divphotoref>div>.photowithmargin,
div.divphotoref>img.photowithmargin,
...
.ui-dialog.ui-corner-all.ui-widget.ui-widget-content.ui-front.ui-draggable {
    ...
    /* Default 101 with ui-jquery, top menu have a z-index of 1000 */
}
...
div.ui-tooltip {
    max-width: <?php print dol_size(700, 'width');
    ?>px !important;
}
...
.websiteformtoolbar {
    ...
    ?>;
    z-index: 1002;
    /* Dolibarr menu is 1001, Website menu is 1002 */
}
...
.treeview ul {
    ...
    margin-top: 0 !important;
    /* margin-bottom: 4px !important; padding-top: 2px !important; */
}
...
div.scroll2 {
    width: <?php print isset($_SESSION['dol_screenwidth']) ? max((int) $_SESSION['dol_screenwidth'] - 830, 450): '450';
    ?>px !important;
}
...
.ecmjqft LI.directory {
    font-weight: normal;
    background: url(<?php echo dol_buildpath($path . '/theme/common/treemenu/folder2.png', 1);
    ?>) left top no-repeat;
...
.ecmjqft LI.expanded {
    font-weight: normal;
    background: url(<?php echo dol_buildpath($path . '/theme/common/treemenu/folder2-expanded.png', 1);
    ?>) left top no-repeat;
    background-position-y: 8px;
}
...
.ecmjqft LI.wait {
    font-weight: normal;
    background: url(<?php echo dol_buildpath('/theme/' . $theme . '/img/working.gif', 1);
    ?>) left top no-repeat;
}
...
.jnotify-container {
    ...
    ?>text-align: center;
    min-width: <?php echo $dol_optimize_smallscreen ? '200' : '480';
    ?>px;
    width: auto;
    max-width: 1024px;
...
.ui-btn-up-c .vsmenudisabled {
    color: #<?php echo $colorshadowtitle;
    ?> !important;
    text-shadow: none !important;
}
...
span.clipboardCPValue.hidewithsize {
    width: 0 !important;
    display: inline-block;
    /* this will be modifiy on the fly by the copy-paste js code in lib_foot.js.php to have copy feature working */
    color: transparent;
    white-space: nowrap;
    overflow-x: hidden;
    vertical-align: middle;
...
@media only screen and (max-width: <?php echo !getDolGlobalString('THEME_ELDY_WITDHOFFSET_FOR_REDUC1') ? round($nbtopmenuentries * 90, 0) + 340 : $conf->global->THEME_ELDY_WITDHOFFSET_FOR_REDUC1; ?>px)

/* reduction 1 */
    {
    div.tmenucenter {
        /* width: <?php echo round(52);
                        ?>px; */
        /* size of viewport */
        white-space: nowrap;
        overflow: hidden;
...
@media only screen and (max-width: <?php echo !getDolGlobalString('THEME_ELDY_WITDHOFFSET_FOR_REDUC2') ? round($nbtopmenuentries * 69, 0) + 130 : $conf->global->THEME_ELDY_WITDHOFFSET_FOR_REDUC2; ?>px)
...
/* reduction 2 */
    ...
    div.tmenucenter {
        max-width: <?php echo max(26, ceil(300 / ($nbtopmenuentriesreal + 2)));
        ?>px;
        /* size of viewport */
        text-overflow: clip;
    }
...
/* reduction 3 */
    ...
    tr.trextrafieldseparator td,
    tr.trextrafields_collapse_last td {
        /* border-bottom: 2px solid var(--colorbackhmenu1) !important; */
        border-bottom: 1px solid var(--colortopbordertitle1) !important;
    }
...
<?php include dol_buildpath($path . '/theme/'. $theme . '/dropdown.inc.php', 0);
